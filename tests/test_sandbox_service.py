...
async def test_create_ssh_session(sandbox_svc, mock_client):
    ...
    result = await sandbox_svc.create_ssh_session("sb1")
...
async def test_create_without_presets(sandbox_svc, mock_client):
    ...
    result = await sandbox_svc.create(name="sb1", image="base")

    assert result == {"name": "sb1", "phase": "provisioning"}
    mock_client.sandboxes.wait_ready.assert_not_called()


async def test_create_happy_path(sandbox_svc, mock_client):
    ...
    result = await sandbox_svc.create(name="sb1", image="base", presets=["pypi"])
...
async def test_create_error_state(sandbox_svc, mock_client):
...
async def test_create_timeout(sandbox_svc, mock_client):
...
async def test_create_preset_partial_failure(sandbox_svc, mock_client):
    """One preset succeeds, another fails — both recorded."""
    mock_client.sandboxes.create.return_value = {"name": "sb1"}
    mock_client.sandboxes.wait_ready.return_value = {"phase": "ready"}
    mock_client.policies.get.return_value = {"policy": {"status": "loaded"}}

    result = await sandbox_svc.create(name="sb1", image="base", presets=["pypi", "bad"])
...
async def test_exec_string_command(sandbox_svc, mock_client):
    ...
    await sandbox_svc.exec("sb1", "echo hello")
...
async def test_exec_list_command(sandbox_svc, mock_client):
...
async def test_get_logs(sandbox_svc, mock_client):
    ...
    result = await sandbox_svc.get_logs("sb1", lines=50, since_ms=1000, min_level="info")
...
async def test_create_with_gpu(sandbox_svc, mock_client):
    """create() forwards gpu=True to client."""
    mock_client.sandboxes.create.return_value = {"name": "sb1", "phase": "provisioning"}

    await sandbox_svc.create(name="sb1", image="base", gpu=True)
...
async def test_create_with_environment(sandbox_svc, mock_client):
    """create() forwards environment dict to client."""
    mock_client.sandboxes.create.return_value = {"name": "sb1"}

    await sandbox_svc.create(name="sb1", environment={"FOO": "bar"})
...
async def test_create_with_providers(sandbox_svc, mock_client):
    """create() forwards providers list to client."""
    mock_client.sandboxes.create.return_value = {"name": "sb1"}

    await sandbox_svc.create(name="sb1", providers=["prov-1", "prov-2"])
...
async def test_create_policy_not_ready_warning(sandbox_svc, mock_client):
...
async def test_exec_forwards_workdir_env_timeout(sandbox_svc, mock_client):
    """exec() forwards workdir, env, and timeout_seconds to client."""
    mock_client.sandboxes.get.return_value = {"id": "abc-123", "name": "sb1"}
    mock_client.sandboxes.exec.return_value = {"exit_code": 0}

    await sandbox_svc.exec("sb1", ["ls"], workdir="/app", env={"KEY": "val"}, timeout_seconds=60)
...
async def test_create_ssh_session_resolves_name(sandbox_svc, mock_client):
    ...
    await sandbox_svc.create_ssh_session("mysb")
...
async def test_create_with_labels_stores_metadata(sandbox_svc_with_meta, mock_client, meta_store):
    """create() with labels and description stores metadata in DB."""
    mock_client.sandboxes.create.return_value = {"name": "sb1", "phase": "provisioning"}

    result = await sandbox_svc_with_meta.create(
        name="sb1",
        image="base",
        gateway_name="gw1",
...
async def test_create_without_metadata_no_store_call(
    sandbox_svc_with_meta, mock_client, meta_store
):
...
async def test_get_without_metadata_returns_defaults(sandbox_svc_with_meta, mock_client):
...
async def test_update_metadata_partial(sandbox_svc_with_meta, mock_client, meta_store):
...
async def test_update_metadata_no_store_raises(sandbox_svc, mock_client):
    ...
    with pytest.raises(RuntimeError, match="Metadata store not configured"):
        await sandbox_svc.update_metadata("gw1", "sb1", description="x")
