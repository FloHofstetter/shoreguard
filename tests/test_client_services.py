...
def test_endpoint_to_dict_all_fields():
...
def test_list_sends_filters(mgr, stub):
    """list() forwards sandbox/limit/offset and converts each entry."""
    result = mgr.list(sandbox="sb1", limit=10, offset=2)
...
def test_get_sends_sandbox_service(mgr, stub):
    """get() forwards sandbox/service and returns the endpoint dict."""
    result = mgr.get(sandbox="sb1", service="web")
    assert stub.request.sandbox == "sb1"
...
def test_expose_sends_all_fields(mgr, stub):
    """expose() forwards sandbox/service/target_port/domain."""
    result = mgr.expose(sandbox="sb1", service="api", target_port=3000, domain=True)
    assert stub.request.sandbox == "sb1"
...
def test_expose_domain_defaults_false(mgr, stub):
    """expose() sends domain=False by default."""
    mgr.expose(sandbox="sb1", service="api", target_port=3000)
...
def test_delete_returns_bool(mgr, stub):
    ...
    result = mgr.delete(sandbox="sb1", service="web")
    assert stub.request.sandbox == "sb1"
    assert stub.request.service == "web"
    assert result is True


def test_delete_false_when_absent():
    ...
    m._timeout = 30.0
    assert m.delete(sandbox="sb1", service="nope") is False
