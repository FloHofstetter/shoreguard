...
async def test_get_client_reconnect_close_fails(svc):
...
async def test_get_client_backoff_escalation(svc):
...
async def test_backoff_prevents_rapid_reconnect(svc):
...
async def test_set_client_none_pops(svc):
...
async def test_set_client_creates_entry(svc):
...
async def test_reset_backoff_existing(svc):
...
async def test_reset_backoff_nonexistent_is_noop(svc):
...
async def test_try_connect_from_registry(svc, registry):
    ...
    with (
        ...
        result = await svc._try_connect(GW)
...
async def test_try_connect_fallback_to_config(svc, monkeypatch):
...
async def test_register_creates_gateway(svc):
    """Register adds gateway to registry and returns record."""
    result = await svc.register(GW, "10.0.0.1:8443", auth_mode="insecure")
...
async def test_register_duplicate_raises(svc):
    ...
    with pytest.raises(ConflictError, match="already registered"):
        await svc.register(GW, "10.0.0.2:8443", auth_mode="insecure")


async def test_register_with_certs(svc):
...
async def test_unregister_removes_gateway(svc):
...
async def test_test_connection_not_registered(svc):
    ...
    with pytest.raises(NotFoundError, match="not registered"):
        await svc.test_connection("nope")
...
async def test_list_all_empty(svc):
...
async def test_get_info_not_registered(svc):
    """get_info for unregistered gateway returns error."""
    result = await svc.get_info("unknown")
...
async def test_get_config_delegates_to_client(svc):
    ...
    result = await svc.get_config(GW)
...
async def test_try_connect_from_registry_allows_private_ip_in_local_mode(
    ...
    with patch(
        ...
        result = await svc._try_connect_from_registry("local-gw", creds)
    assert result is mock_client


async def test_try_connect_from_registry_relaxes_mtls_for_local_plaintext(
    svc, registry, monkeypatch
):
    ...
    with patch(
        ...
        result = await svc._try_connect_from_registry("local-plain", creds)
    assert result is mock_client
    assert from_creds.call_args.kwargs["require_mtls"] is False


async def test_try_connect_from_registry_keeps_mtls_default_without_local_mode(
    svc, registry, monkeypatch
):
    ...
    with (
        ...
        result = await svc._try_connect_from_registry("remote-plain", creds)
    assert result is mock_client
...
async def test_try_connect_from_registry_allows_public_ip(svc, registry):
    ...
    with (
        ...
        result = await svc._try_connect_from_registry("public-gw", creds)
    assert result is mock_client


async def test_check_all_health_empty_registry(svc, registry):
    """check_all_health returns early when no gateways are registered."""
    await svc.check_all_health()  # Should not raise
...
async def test_check_all_health_db_error_does_not_stop_loop(svc, registry):
...
class TestDeriveStatusExhaustive:
    ...
    async def test_connected_overrides_any_last_status(self):
        for status in (None, "healthy", "degraded", "unknown", "unreachable", "offline", ""):
            assert _derive_status(True, status) == "connected"

    async def test_disconnected_healthy_is_unreachable(self):
        assert _derive_status(False, "healthy") == "unreachable"

    async def test_disconnected_degraded_is_unreachable(self):
        assert _derive_status(False, "degraded") == "unreachable"

    async def test_disconnected_none_is_offline(self):
        assert _derive_status(False, None) == "offline"

    async def test_disconnected_unknown_is_offline(self):
        assert _derive_status(False, "unknown") == "offline"

    async def test_disconnected_empty_string_is_offline(self):
        assert _derive_status(False, "") == "offline"

    async def test_disconnected_unreachable_is_offline(self):
        """'unreachable' as last_status is NOT in the healthy/degraded set."""
        assert _derive_status(False, "unreachable") == "offline"

    async def test_disconnected_random_string_is_offline(self):
        assert _derive_status(False, "something_else") == "offline"

    async def test_return_type_is_str(self):
...
class TestTryConnectFromConfig:
    ...
    async def test_success_returns_client(self, svc):
        mock_client = AsyncMock()
        mock_client.health.return_value = {"status": "healthy"}
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
            ...
            result = await svc._try_connect_from_config(GW)
        assert result is mock_client
        mock_client.health.assert_called_once()

    async def test_from_active_cluster_grpc_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
            side_effect=grpc.RpcError(),
        ):
            assert await svc._try_connect_from_config(GW) is None

    async def test_from_active_cluster_oserror_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_from_active_cluster_connection_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_from_active_cluster_timeout_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_from_active_cluster_key_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_from_active_cluster_value_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_from_active_cluster_json_decode_error_returns_none(self, svc):
        import json
    ...
    async def test_from_active_cluster_gateway_not_connected_error_returns_none(self, svc):
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_health_fail_returns_none_and_closes(self, svc):
        mock_client = AsyncMock()
        mock_client.health.side_effect = grpc.RpcError()
        with patch(
            "shoreguard.services.gateway.ShoreGuardClient.from_active_cluster",
    ...
    async def test_health_fail_oserror_returns_none(self, svc):
        mock_client = AsyncMock()
    ...
    async def test_health_fail_connection_error_returns_none(self, svc):
        mock_client = AsyncMock()
    ...
    async def test_health_fail_timeout_error_returns_none(self, svc):
        mock_client = AsyncMock()
    ...
    async def test_health_fail_close_raises_is_ok(self, svc):
    ...
    async def test_health_return_value_not_used_directly(self, svc):
...
class TestTryConnectFromRegistry:
    ...
    async def test_from_credentials_called_with_bytes_only(self, svc):
    ...
    async def test_from_credentials_called_with_bytes(self, svc):
    ...
    async def test_from_credentials_missing_optional_creds(self, svc):
    ...
    async def test_connection_error_returns_none(self, svc):
        creds = {"endpoint": "host:443"}
        with (
            patch("shoreguard.services.gateway.is_private_ip", return_value=False),
            patch(
            ...
            assert await svc._try_connect_from_registry("gw", creds) is None

    async def test_timeout_error_returns_none(self, svc):
        creds = {"endpoint": "host:443"}
        with (
            patch("shoreguard.services.gateway.is_private_ip", return_value=False),
            patch(
    ...
    async def test_health_fail_oserror(self, svc):
        creds = {"endpoint": "host:443"}
        mock_client = AsyncMock()
        mock_client.health.side_effect = OSError("fail")
        with (
    ...
    async def test_health_fail_connection_error(self, svc):
        creds = {"endpoint": "host:443"}
        mock_client = AsyncMock()
        mock_client.health.side_effect = ConnectionError("fail")
        with (
    ...
    async def test_health_fail_timeout_error(self, svc):
        creds = {"endpoint": "host:443"}
        mock_client = AsyncMock()
        mock_client.health.side_effect = TimeoutError("fail")
        with (
    ...
    async def test_health_fail_close_raises_grpc(self, svc):
        creds = {"endpoint": "host:443"}
        mock_client = AsyncMock()
        mock_client.health.side_effect = grpc.RpcError()
    ...
    async def test_health_fail_close_raises_oserror(self, svc):
        creds = {"endpoint": "host:443"}
        mock_client = AsyncMock()
        mock_client.health.side_effect = grpc.RpcError()
        mock_client.close.side_effect = OSError("close fail")
...
class TestCheckAllHealth:
    ...
    async def test_empty_registry_returns_early(self, svc, registry):
...
class TestSetClient:
    ...
    async def test_set_client_stores_exact_client(self, svc):
        mock = AsyncMock()
        svc.set_client(mock, name=GW)
        assert svc._clients[GW].client is mock

    async def test_set_client_resets_backoff_to_zero(self, svc):
    ...
    async def test_set_client_none_removes_entry_completely(self, svc):
    ...
    async def test_set_client_none_on_missing_is_noop(self, svc):
    ...
    async def test_set_client_overwrites_existing(self, svc):
    ...
    async def test_set_client_creates_new_entry_if_needed(self, svc):
    ...
    async def test_set_client_preserves_other_gateways(self, svc):
        mock1 = AsyncMock()
        mock2 = AsyncMock()
...
class TestResetBackoff:
    ...
    async def test_resets_backoff_to_zero(self, svc):
        entry = gw_module._ClientEntry()
        entry.backoff = 60.0
        svc._clients[GW] = entry
        svc.reset_backoff(name=GW)
    ...
    async def test_resets_last_attempt_to_zero(self, svc):
        entry = gw_module._ClientEntry()
    ...
    async def test_noop_for_missing_gateway(self, svc):
        """No error when gateway not in _clients."""
        svc.reset_backoff(name="missing")

    async def test_noop_for_empty_string(self, svc):
        """Empty name should be a no-op (guard: `if gw_name and ...`)."""
        svc.reset_backoff(name="")

    async def test_both_fields_reset_together(self, svc):
        entry = gw_module._ClientEntry()
        entry.backoff = 30.0
    ...
    async def test_does_not_remove_entry(self, svc):
...
class TestGetInfo:
    ...
    async def test_not_registered_returns_exact_dict(self, svc):
        result = await svc.get_info("nope")
...
class TestGetCachedClient:
    ...
    async def test_returns_none_when_no_entry(self, svc):
        assert svc.get_cached_client("missing") is None

    async def test_returns_none_when_client_is_none(self, svc):
        entry = gw_module._ClientEntry()
        entry.client = None
        svc._clients[GW] = entry
        assert svc.get_cached_client(GW) is None

    async def test_returns_exact_client(self, svc):
        mock = AsyncMock()
        entry = gw_module._ClientEntry()
        entry.client = mock
        svc._clients[GW] = entry
    ...
    async def test_does_not_modify_state(self, svc):
        mock = AsyncMock()
        entry = gw_module._ClientEntry()
        entry.client = mock
        entry.backoff = 42.0
...
class TestGetClientBackoff:
    ...
    async def test_initial_backoff_is_backoff_min(self, svc):
        from shoreguard.settings import get_settings
    ...
    async def test_second_failure_doubles_backoff(self, svc):
        from shoreguard.settings import get_settings
    ...
    async def test_backoff_caps_at_max(self, svc):
        from shoreguard.settings import get_settings
    ...
    async def test_successful_reconnect_clears_backoff(self, svc):
    ...
    async def test_within_backoff_window_skips_connect(self, svc):
    ...
    async def test_last_attempt_updated_on_each_try(self, svc):
        import time
...
class TestRegister:
    ...
    async def test_register_connected_true(self, svc):
        mock_client = AsyncMock()
        mock_client.health.return_value = {"status": "ok"}

        with patch.object(svc, "_try_connect", return_value=mock_client):
            result = await svc.register("rg", "host:443", auth_mode="insecure")

        assert result["connected"] is True
        assert result["status"] == "connected"

    async def test_register_connected_false(self, svc):
        result = await svc.register("rg", "host:443", auth_mode="insecure")
        assert result["connected"] is False
        assert result["status"] == "unreachable"

    async def test_register_returns_name_and_endpoint(self, svc):
        result = await svc.register("myname", "myhost:443", auth_mode="insecure")
...
class TestRegistryProperty:
    async def test_returns_registry(self, svc, registry):
