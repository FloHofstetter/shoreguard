...
class _FakeStub:
    ...
    async def ListProviders(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(providers=[_make_provider("p1"), _make_provider("p2")])

    async def GetProvider(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(provider=_make_provider("prov-1"))

    async def CreateProvider(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(provider=_make_provider("new-prov"))

    async def UpdateProvider(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(provider=_make_provider("prov-1", "openai"))

    async def DeleteProvider(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(deleted=True)

    async def ConfigureProviderRefresh(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(status=_make_refresh_status(req.provider, req.credential_key))

    async def GetProviderRefreshStatus(self, req, timeout=None):
        self.request = req
    ...
    async def RotateProviderCredential(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(status=_make_refresh_status(req.provider, req.credential_key))

    async def DeleteProviderRefresh(self, req, timeout=None):
        self.request = req
        return SimpleNamespace(deleted=True)
...
async def test_list_sends_limit_offset(mgr, stub):
...
async def test_create_sends_credentials(mgr, stub):
...
async def test_update_sends_name_and_type(mgr, stub):
    """update() builds Provider proto with new type and sends it."""
    result = await mgr.update(name="prov-1", provider_type="openai")
...
async def test_delete_returns_bool(mgr, stub):
    ...
    result = await mgr.delete("prov-1")
...
async def test_configure_refresh_maps_strategy_and_material(mgr, stub):
    """configure_refresh() sends provider/key/strategy/material and returns status."""
    result = await mgr.configure_refresh(
        provider="prov-1",
...
async def test_configure_refresh_rejects_unknown_strategy(mgr):
    """configure_refresh() raises ValueError for an unknown strategy."""
    with pytest.raises(ValueError, match="Unknown refresh strategy"):
        await mgr.configure_refresh(provider="p", credential_key="k", strategy="bogus")


async def test_configure_refresh_omits_expiry_when_none(mgr, stub):
    """configure_refresh() leaves expires_at_ms unset when not provided."""
    await mgr.configure_refresh(provider="p", credential_key="k", strategy="static")
    assert stub.request.HasField("expires_at_ms") is False


async def test_get_refresh_status_lists_entries(mgr, stub):
    """get_refresh_status() forwards provider/key and converts each entry."""
    result = await mgr.get_refresh_status("prov-1", credential_key="")
    assert stub.request.provider == "prov-1"
...
async def test_rotate_credential_returns_status(mgr, stub):
    """rotate_credential() sends provider/key and returns the status dict."""
    result = await mgr.rotate_credential(provider="prov-1", credential_key="KEY_A")
    assert stub.request.provider == "prov-1"
...
async def test_delete_refresh_returns_bool(mgr, stub):
    """delete_refresh() sends provider/key and returns deleted bool."""
    result = await mgr.delete_refresh(provider="prov-1", credential_key="KEY_A")
    assert stub.request.provider == "prov-1"
    assert stub.request.credential_key == "KEY_A"
    assert result is True


async def test_strategy_round_trip():
...
async def test_provider_to_dict_all_fields():
...
async def test_create_with_config(mgr, stub):
...
async def test_update_with_credentials_and_config(mgr, stub):
...
async def test_list_returns_multiple_providers_with_correct_fields(mgr, stub):
...
class TestProviderToDictMutations:
    ...
    async def test_empty_credentials_and_config(self):
        provider = datamodel_pb2.Provider(
    ...
    async def test_each_field_maps_correctly(self):
        provider = datamodel_pb2.Provider(
    ...
    async def test_dict_keys_exact(self):
        provider = datamodel_pb2.Provider(
            metadata=datamodel_pb2.ObjectMeta(id="i", name="n"),
            type="t",
        )
...
class TestProviderManagerMutations:
    ...
    async def test_create_no_credentials_empty(self):
        class _Stub(_FakeStub):
            async def CreateProvider(self, req, timeout=None):
                self.request = req
    ...
    async def test_update_no_credentials_empty(self):
        class _Stub(_FakeStub):
            async def UpdateProvider(self, req, timeout=None):
                self.request = req
                return SimpleNamespace(provider=_make_provider("p"))
        ...
        m._stub = s  # type: ignore[assignment]
        m._timeout = 30.0
        await m.update(name="p")
        assert dict(s.request.provider.credentials) == {}
        assert dict(s.request.provider.config) == {}

    async def test_update_default_type_empty(self):
        class _Stub(_FakeStub):
            async def UpdateProvider(self, req, timeout=None):
                self.request = req
                return SimpleNamespace(provider=_make_provider("p"))
    ...
    async def test_delete_sends_name_in_request(self):
        class _Stub(_FakeStub):
            async def DeleteProvider(self, req, timeout=None):
                self.request = req
                return SimpleNamespace(deleted=True)
        ...
        m._stub = s  # type: ignore[assignment]
        m._timeout = 30.0
        await m.delete("my-prov")
        assert s.request.name == "my-prov"

    async def test_create_provider_type_set(self):
        class _Stub(_FakeStub):
            async def CreateProvider(self, req, timeout=None):
                self.request = req
                return SimpleNamespace(provider=_make_provider("p"))
        ...
        m._stub = s  # type: ignore[assignment]
        m._timeout = 30.0
        await m.create(name="p", provider_type="openai")
