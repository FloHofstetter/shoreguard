...
class TestDiscoverDomain:
    async def test_basic(self, discovery_setup):
    ...
    async def test_sorted_by_priority(self, discovery_setup):
        svc, _, _ = discovery_setup
        with _patch_resolve(
            [
    ...
    async def test_timeout(self, discovery_setup):
        svc, _, _ = discovery_setup
        with patch(
            "dns.resolver.Resolver.resolve",
    ...
    async def test_generic_dns_error(self, discovery_setup):
        svc, _, _ = discovery_setup
        with patch(
            "dns.resolver.Resolver.resolve",
...
class TestDiscoverAll:
    async def test_iterates_settings_domains(self, discovery_setup):
        svc, _, _ = discovery_setup
    ...
    async def test_explicit_override(self, discovery_setup):
        svc, _, _ = discovery_setup
        with patch.object(svc, "discover_domain", return_value=[]) as mock:
...
class TestAutoRegister:
    async def test_registers_new_endpoint(self, discovery_setup):
    ...
    async def test_skips_already_registered(self, discovery_setup):
        ...
        await gw_svc.register("preexisting", "gw.example.com:30051", scheme="grpc+tls")
        ep = DiscoveredEndpoint("gw.example.com", 30051, 10, 5, "example.com")
        result = await svc.auto_register([ep])
    ...
    async def test_skips_private_ip(self, discovery_setup):
        svc, _, _ = discovery_setup
    ...
    async def test_allows_svc_cluster_local(self, discovery_setup):
        svc, registry, _ = discovery_setup
    ...
    async def test_dedupes_within_batch(self, discovery_setup):
        ...
        result = await svc.auto_register([ep1, ep2])
        assert len(result["registered"]) == 1
        assert len(result["skipped"]) == 1


class TestRunOnce:
    async def test_run_once_persists(self, discovery_setup):
        svc, registry, _ = discovery_setup
    ...
    async def test_run_once_explicit_domains(self, discovery_setup):
        svc, _, _ = discovery_setup
        with patch.object(svc, "discover_domain", return_value=[]) as mock:
            await svc.run_once(domains=["custom.example.com"])
        mock.assert_called_with("custom.example.com")


class TestStatus:
    async def test_status_initial(self, discovery_setup):
        svc, _, _ = discovery_setup
        s = svc.status()
    ...
    async def test_status_after_run(self, discovery_setup):
        svc, _, _ = discovery_setup
        with _patch_resolve([_srv("gw.example.com", 30051)]):
            await svc.run_once()
        s = svc.status()
