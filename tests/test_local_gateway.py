...
async def test_diagnostics_docker_not_installed(mock_run, mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_diagnostics_docker_running(mock_run, mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_diagnostics_docker_permission_denied(mock_run, mock_which, mgr):
...
async def test_start_already_running(mock_status, mock_docker, mgr):
    """Start when container is already running."""
    mock_docker.return_value = True
    mock_status.return_value = "running"
    result = await mgr.start(GW)
    assert result["success"] is True
    assert "already running" in result["output"]


@patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
async def test_stop_already_stopped(mock_status, mgr):
    """Stop when container is not running."""
    mock_status.return_value = "exited"
    result = await mgr.stop(GW)
    assert result["success"] is True
    assert "already stopped" in result["output"]


@patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
async def test_start_docker_not_running(mock_docker, mgr):
...
async def test_start_exited_container(mock_start, mock_status, mock_docker, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
async def test_create_openshell_not_found(mock_which, mgr):
    """Create fails when openshell CLI is missing."""
    mock_which.return_value = None
    result = await mgr.create("new-gw")
    assert result["success"] is False
...
@patch("shoreguard.services.local_gateway.shutil.which")
@patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
async def test_create_docker_not_running(mock_docker, mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
async def test_destroy_openshell_not_found(mock_which, mgr):
    """Destroy fails when openshell CLI is missing."""
    mock_which.return_value = None
    result = await mgr.destroy("test-gw")
    assert result["success"] is False
    assert "openshell CLI not found" in result["error"]
...
async def test_destroy_no_resources(mock_run, mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
async def test_destroy_with_resources_no_force(mock_which, mgr):
    """Destroy blocked when gateway has resources and force=False."""
    mock_which.return_value = "/usr/bin/openshell"

    mock_client = MagicMock()
    mock_client.sandboxes = AsyncMock()
    mock_client.providers = AsyncMock()
    mock_client.sandboxes.list.return_value = [{"name": "sb1"}]
    mock_client.providers.list.return_value = []

    with patch.object(mgr, "_get_client_if_connected", new=AsyncMock(return_value=mock_client)):
        result = await mgr.destroy("test-gw", force=False)
...
@patch("shoreguard.services.local_gateway.shutil.which")
@patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
async def test_destroy_with_resources_force(mock_run, mock_which, mgr):
    ...
    with patch.object(mgr, "_get_client_if_connected", new=AsyncMock(return_value=mock_client)):
        result = await mgr.destroy("test-gw", force=True)
...
@patch("shoreguard.services.local_gateway.shutil.which")
@patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
async def test_destroy_sandbox_delete_error_continues(mock_run, mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which")
async def test_destroy_resource_listing_failure_no_force(mock_which, mgr):
...
async def test_get_port_for_gateway_no_metadata(mgr, config_dir):
    """Returns None when no metadata file exists."""
    port = mgr._get_port_for_gateway("nonexistent")
    assert port is None


async def test_get_port_for_gateway_with_metadata(mgr, config_dir):
...
async def test_get_port_for_gateway_invalid_json(mgr, config_dir):
...
async def test_next_free_port_empty(mgr, config_dir):
    """Returns start port when no gateways exist."""
    port = mgr._next_free_port()
    assert port == 8080


async def test_next_free_port_skips_used(mgr, config_dir):
...
async def test_next_free_port_overflow_raises(mgr, config_dir):
...
@patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
async def test_find_port_blocker_none(mock_status, mgr, config_dir):
...
@patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
async def test_find_port_blocker_found(mock_status, mgr, config_dir):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_get_container_status_not_found(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_start_container_success(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_start_container_failure(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_start_container_timeout(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_stop_container_success(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_stop_container_failure(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_docker_stop_container_timeout(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_check_docker_daemon_running(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_check_docker_daemon_not_running(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_run_openshell_success(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_run_openshell_failure(mock_run, mgr):
...
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_run_openshell_timeout(mock_run, mgr):
...
async def test_destroy_list_failure_without_force(mock_which, mgr):
...
@patch("shoreguard.services.local_gateway.shutil.which", return_value="/usr/bin/openshell")
@patch("shoreguard.services.local_gateway.subprocess.run")
async def test_destroy_list_failure_with_force_proceeds(mock_run, mock_which, mgr):
...
class TestDiagnosticsExhaustive:
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_not_installed_all_defaults(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_user_from_environ(self, mock_run, mock_which, mgr, monkeypatch):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_user_defaults_to_unknown(self, mock_run, mock_which, mgr, monkeypatch):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_running_all_fields(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_daemon_not_running_error(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_not_responding_error(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_unknown_stderr_error(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_info_raises_exception(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_groups_command_fails(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_groups_command_raises_exception(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_openshell_installed_version_fails(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_openshell_version_raises_exception(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_info_called_with_correct_args(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_permission_denied_sets_daemon_running_true(self, mock_run, mock_which, mgr):
...
class TestStartExhaustive:
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    async def test_start_docker_not_running_exact_error(self, mock_docker, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_start_already_running_exact_output(self, mock_status, mock_docker, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_start_running_tries_get_client(self, mock_status, mock_docker, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_start_running_get_client_error_still_success(
        self, mock_status, mock_docker, mgr
    ):
    ...
    async def test_start_exited_port_conflict(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_port_for_gateway")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_no_port_starts_container(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_no_blocker_starts_container(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_success_resets_backoff(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_failure_no_backoff_reset(
        self, mock_sleep, mock_start, mock_status, mock_docker, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_retries_connection(
        self, mock_sleep, mock_start, mock_status, mock_docker, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_start_container")
    @patch("shoreguard.services.local_gateway.asyncio.sleep", new_callable=AsyncMock)
    async def test_start_exited_breaks_on_first_success(
        self, mock_sleep, mock_start, mock_status, mock_docker, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_start_no_container_openshell_not_found(
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_start_no_container_runs_openshell(
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_start_openshell_success_tries_get_client(
        self, mock_run, mock_which, mock_status, mock_docker, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_start_openshell_failure_skips_get_client(
        self, mock_run, mock_which, mock_status, mock_docker, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_start_status_created_treated_like_exited(self, mock_status, mock_docker, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_start_status_dead_treated_like_exited(self, mock_status, mock_docker, mgr):
...
class TestStopExhaustive:
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_stop_already_stopped_exact(self, mock_status, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_stop_status_none_treated_as_stopped(self, mock_status, mgr):
    ...
    async def test_stop_running_calls_docker_stop(self, mock_stop, mock_status, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._docker_stop_container")
    async def test_stop_failure_does_not_clear_client(self, mock_stop, mock_status, mgr):
...
class TestRestart:
    async def test_restart_calls_stop_then_start(self, mgr):
...
class TestCreateExhaustive:
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_create_no_openshell_exact(self, mock_which, mgr):
        """Exact error when openshell not found."""
        mock_which.return_value = None
        result = await mgr.create("gw1")
        assert result == {"success": False, "error": "openshell CLI not found"}

    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    async def test_create_no_docker_exact(self, mock_docker, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._check_docker_daemon")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    async def test_create_port_conflict_exact(self, mock_blocker, mock_docker, mock_which, mgr):
        """Port conflict returns exact error with gateway name."""
        mock_which.return_value = "/usr/bin/openshell"
        mock_docker.return_value = True
        mock_blocker.return_value = "other-gw"
        result = await mgr.create("gw1", port=8080)
        assert result["success"] is False
        assert "Port 8080" in result["error"]
        assert '"other-gw"' in result["error"]
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_no_port_uses_next_free(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._next_free_port")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_port_zero_uses_next_free(
        self, mock_run, mock_nfp, mock_docker, mock_which, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_with_port_no_blocker(
        ...
        mock_blocker.return_value = None
        mock_run.return_value = {"success": False, "error": "fail"}
        await mgr.create("gw1", port=9090)
        mock_run.assert_called_once_with(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_with_remote_host(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
        ...
        mock_blocker.return_value = None
        mock_run.return_value = {"success": False, "error": "fail"}
        await mgr.create("gw1", port=8080, remote_host="server.example.com")
        mock_run.assert_called_once_with(
            [
                "gateway",
                "start",
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_with_gpu(self, mock_run, mock_blocker, mock_docker, mock_which, mgr):
        """GPU flag adds --gpu arg."""
        mock_which.return_value = "/usr/bin/openshell"
        mock_docker.return_value = True
        mock_blocker.return_value = None
        mock_run.return_value = {"success": False, "error": "fail"}
        await mgr.create("gw1", port=8080, gpu=True)
        mock_run.assert_called_once_with(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_with_remote_and_gpu(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
        ...
        mock_blocker.return_value = None
        mock_run.return_value = {"success": False, "error": "fail"}
        await mgr.create("gw1", port=8080, remote_host="host", gpu=True)
        mock_run.assert_called_once_with(
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_success_returns_info_with_gpu(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
        ...
        with (
            ...
            result = await mgr.create("gw1", port=8080, gpu=True)
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_success_gpu_false(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
        ...
        with (
            ...
            result = await mgr.create("gw1", port=8080, gpu=False)
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_failure_returns_run_result(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._find_port_blocker")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_create_negative_port_uses_next_free(
        self, mock_run, mock_blocker, mock_docker, mock_which, mgr
    ):
...
class TestDestroyExhaustive:
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_no_openshell_exact(self, mock_which, mgr):
        """Exact return value when openshell missing."""
        mock_which.return_value = None
        result = await mgr.destroy("gw1")
        assert result == {"success": False, "error": "openshell CLI not found"}

    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_no_client_runs_openshell(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_resources_no_force_sandboxes_only(self, mock_which, mgr):
        ...
        mock_client.providers.list.return_value = []
        with patch.object(mgr, "_get_client_if_connected", new=AsyncMock(return_value=mock_client)):
            result = await mgr.destroy("gw1", force=False)
        assert result["success"] is False
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_resources_no_force_providers_only(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_resources_no_force_both(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_resource_with_id_fallback(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_resource_no_name_no_id(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_force_deletes_sandboxes_and_providers(self, mock_run, mock_which, mgr):
        ...
        mock_client.providers.list.return_value = [{"name": "p1"}]
        with patch.object(mgr, "_get_client_if_connected", new=AsyncMock(return_value=mock_client)):
            await mgr.destroy("gw1", force=True)
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_force_skips_empty_name_sandboxes(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_force_provider_delete_error_continues(self, mock_run, mock_which, mgr):
        ...
        with patch.object(mgr, "_get_client_if_connected", new=AsyncMock(return_value=mock_client)):
            result = await mgr.destroy("gw1", force=True)
        assert mock_client.providers.delete.call_count == 2
        assert result["success"] is True

    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_sandboxes_none_no_force(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    async def test_destroy_providers_none_no_force(self, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_both_none_with_force_proceeds(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_empty_resources_no_force_proceeds(self, mock_run, mock_which, mgr):
    ...
    @patch("shoreguard.services.local_gateway.shutil.which")
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._run_openshell")
    async def test_destroy_clears_client_before_openshell(self, mock_run, mock_which, mgr):
        """set_client(None) is called regardless of resource state."""
        mock_which.return_value = "/usr/bin/openshell"
        mock_run.return_value = {"success": True, "output": "ok"}
        with patch.object(mgr._gw, "set_client") as mock_sc:
            await mgr.destroy("gw1")
            mock_sc.assert_called_once_with(None, name="gw1")
...
class TestDockerHelpersExact:
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_start_exact_args(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_start_success_exact_output(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_start_failure_empty_stderr_uses_exit_code(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_start_timeout_exact(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_start_oserror(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_stop_exact_args(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_stop_success_exact_output(self, mock_run, mgr):
        """Exact output on success."""
        mock_run.return_value = MagicMock(returncode=0, stdout="ok")
        result = mgr._docker_stop_container("my-gw")
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_stop_failure_empty_stderr(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_stop_timeout_exact(self, mock_run, mgr):
        """Timeout returns exact message."""
        mock_run.side_effect = subprocess.TimeoutExpired(cmd="docker", timeout=30)
        result = mgr._docker_stop_container("my-gw")
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_docker_stop_oserror(self, mock_run, mgr):
        """OSError returns str(e)."""
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_get_container_status_exact_args(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_check_docker_daemon_nonzero_returns_false(self, mock_run, mgr):
        """Non-zero return code means daemon not running."""
        mock_run.return_value = MagicMock(returncode=1)
        assert mgr._check_docker_daemon() is False

    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_check_docker_daemon_subprocess_error(self, mock_run, mgr):
        """SubprocessError returns False."""
        mock_run.side_effect = subprocess.SubprocessError("fail")
        assert mgr._check_docker_daemon() is False

    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_check_docker_daemon_oserror(self, mock_run, mgr):
...
class TestRunOpenshellExact:
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_exact_args(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_success_stdout_preferred(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_success_stderr_fallback(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_failure_stderr_preferred(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_failure_stdout_fallback(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_failure_exit_code_fallback(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_timeout_with_value(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_oserror(self, mock_run, mgr):
        """OSError returns str(e)."""
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_default_timeout(self, mock_run, mgr):
    ...
    @patch("shoreguard.services.local_gateway.subprocess.run")
    async def test_run_openshell_success_strips_output(self, mock_run, mgr):
        """Output is stripped."""
        mock_run.return_value = MagicMock(returncode=0, stdout="  ok  \n", stderr="")
        result = mgr._run_openshell(["test"])
        assert result["output"] == "ok"
...
class TestGetClientIfConnected:
    ...
    async def test_returns_cached_client(self, mgr):
        """Returns cached client when available."""
        mock_client = MagicMock()
        with patch.object(mgr._gw, "get_cached_client", return_value=mock_client):
            result = await mgr._get_client_if_connected("gw1")
        assert result is mock_client

    async def test_cached_none_tries_get_client(self, mgr):
    ...
    async def test_get_client_raises_returns_none(self, mgr):
    ...
    async def test_get_client_grpc_error_returns_none(self, mgr):
    ...
    async def test_get_client_oserror_returns_none(self, mgr):
        """OSError returns None."""
        with (
            patch.object(mgr._gw, "get_cached_client", return_value=None),
...
class TestListResourcesSafe:
    ...
    async def test_returns_list_on_success(self, mgr):
        """Returns list when call succeeds."""
        fn = AsyncMock(return_value=[{"name": "a"}])
        result = await mgr._list_resources_safe(fn)
        assert result == [{"name": "a"}]
        fn.assert_called_once()

    async def test_returns_empty_list_on_success(self, mgr):
        """Returns empty list when no resources."""
        fn = AsyncMock(return_value=[])
        result = await mgr._list_resources_safe(fn)
        assert result == []

    async def test_returns_none_on_grpc_error(self, mgr):
        """grpc.RpcError returns None."""
        fn = MagicMock(side_effect=grpc.RpcError())
        result = await mgr._list_resources_safe(fn)
        assert result is None

    async def test_returns_none_on_oserror(self, mgr):
        """OSError returns None."""
        fn = MagicMock(side_effect=OSError("fail"))
        result = await mgr._list_resources_safe(fn)
        assert result is None

    async def test_returns_none_on_connection_error(self, mgr):
...
class TestPortManagementExact:
    ...
    async def test_get_port_for_gateway_returns_exact_value(self, mgr, config_dir):
    ...
    async def test_get_port_for_gateway_no_port_key(self, mgr, config_dir):
    ...
    async def test_next_free_port_with_explicit_start(self, mgr, config_dir):
        """Explicit start parameter is respected."""
        assert mgr._next_free_port(start=9000) == 9000

    async def test_next_free_port_skips_multiple(self, mgr, config_dir):
    ...
    async def test_next_free_port_overflow_exact_message(self, mgr, config_dir):
        """Exact error message on overflow."""
        with patch.object(mgr, "_get_used_ports", return_value=set(range(8080, 65536))):
            with pytest.raises(RuntimeError, match="No free ports available in valid range"):
                mgr._next_free_port()

    async def test_get_used_ports_no_gateways_dir(self, mgr, config_dir):
        """Returns empty set when gateways dir doesn't exist."""
        result = mgr._get_used_ports()
        assert result == set()

    async def test_get_used_ports_with_gateways(self, mgr, config_dir):
    ...
    async def test_get_used_ports_skips_files(self, mgr, config_dir):
    ...
    async def test_get_used_ports_skips_none_ports(self, mgr, config_dir):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_find_port_blocker_skips_self(self, mock_status, mgr, config_dir):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_find_port_blocker_not_running(self, mock_status, mgr, config_dir):
    ...
    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_find_port_blocker_different_port(self, mock_status, mgr, config_dir):
    ...
    async def test_find_port_blocker_no_gateways_dir(self, mgr, config_dir):
        """No gateways dir returns None."""
        result = mgr._find_port_blocker(GW, 8080)
        assert result is None

    @patch("shoreguard.services.local_gateway.LocalGatewayManager._get_container_status")
    async def test_find_port_blocker_skips_files(self, mock_status, mgr, config_dir):
...
class TestContainerName:
    async def test_container_name_format(self, mgr):
