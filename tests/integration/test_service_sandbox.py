...
async def test_service_list(sandbox_service):
    """await SandboxService.list() returns a list."""
    result = await sandbox_service.list()
    assert isinstance(result, list)


async def test_service_create_no_presets(sandbox_service, sg_client):
    """Create a sandbox without presets via the service layer."""
    result = await sandbox_service.create(name="integ-svc-basic")
...
async def test_service_exec_string(sandbox_service, ready_sandbox):
    """exec() with a string command uses shlex parsing."""
    result = await sandbox_service.exec(ready_sandbox["name"], "echo integration-test")

    assert result["exit_code"] == 0
    assert "integration-test" in result["stdout"]


async def test_service_create_with_preset(sandbox_service, sg_client):
    """Create a sandbox with a preset applied."""
    import uuid

    name = f"integ-preset-{uuid.uuid4().hex[:8]}"
    result = await sandbox_service.create(name=name, presets=["pypi"])
