...
class TestRegisterWithMetadata:
    async def test_register_with_description(self, registry):
        gw = await registry.register("gw1", "10.0.0.1:8443", description="Production EU-West")
        assert gw["description"] == "Production EU-West"

    async def test_register_with_labels(self, registry):
        gw = await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod", "team": "ml"})
        assert gw["labels"] == {"env": "prod", "team": "ml"}

    async def test_register_with_both(self, registry):
        gw = await registry.register(
            "gw1",
            "10.0.0.1:8443",
            description="Staging",
            labels={"env": "staging"},
    ...
    async def test_register_without_metadata(self, registry):
        gw = await registry.register("gw1", "10.0.0.1:8443")
...
class TestToDict:
    async def test_labels_in_dict(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod"})
        gw = await registry.get("gw1")
        assert gw is not None
        assert gw["labels"] == {"env": "prod"}

    async def test_description_in_dict(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", description="Test gateway")
        gw = await registry.get("gw1")
        assert gw is not None
        assert gw["description"] == "Test gateway"

    async def test_corrupt_labels_json(self, registry):
        ...
        gw_dict = await registry.get("gw1")
...
class TestUpdateGatewayMetadata:
    async def test_update_description_only(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod"})
        result = await registry.update_gateway_metadata("gw1", description="Updated desc")
        assert result is not None
        assert result["description"] == "Updated desc"
        assert result["labels"] == {"env": "prod"}  # Labels unchanged

    async def test_update_labels_only(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", description="Original")
    ...
    async def test_update_both(self, registry):
        await registry.register("gw1", "10.0.0.1:8443")
    ...
    async def test_clear_description(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", description="Will be cleared")
        result = await registry.update_gateway_metadata("gw1", description=None)
        assert result is not None
        assert result["description"] is None

    async def test_clear_labels(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod"})
    ...
    async def test_not_found(self, registry):
        result = await registry.update_gateway_metadata("nonexistent", description="test")
...
class TestListAllFiltering:
    async def test_filter_by_single_label(self, registry):
        await registry.register("prod-gw", "10.0.0.1:8443", labels={"env": "prod"})
        await registry.register("staging-gw", "10.0.0.2:8443", labels={"env": "staging"})
        await registry.register("no-labels", "10.0.0.3:8443")

        result = await registry.list_all(labels_filter={"env": "prod"})
        assert len(result) == 1
        assert result[0]["name"] == "prod-gw"

    async def test_filter_by_multiple_labels(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod", "team": "ml"})
        await registry.register("gw2", "10.0.0.2:8443", labels={"env": "prod", "team": "infra"})
        await registry.register("gw3", "10.0.0.3:8443", labels={"env": "staging"})

        result = await registry.list_all(labels_filter={"env": "prod", "team": "ml"})
        assert len(result) == 1
        assert result[0]["name"] == "gw1"

    async def test_filter_no_match(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod"})
        result = await registry.list_all(labels_filter={"env": "nonexistent"})
        assert len(result) == 0

    async def test_no_filter_returns_all(self, registry):
        await registry.register("gw1", "10.0.0.1:8443", labels={"env": "prod"})
...
class TestValidationHelpers:
    async def test_validate_labels_valid(self):
    ...
    async def test_validate_labels_invalid_key(self):
    ...
    async def test_validate_labels_too_many(self):
        from fastapi import HTTPException
    ...
    async def test_validate_labels_value_too_long(self):
        from fastapi import HTTPException
    ...
    async def test_validate_description_valid(self):
    ...
    async def test_validate_description_none(self):
        from shoreguard.api.validation import validate_description as _validate_description
