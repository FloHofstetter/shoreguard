...
async def _make(svc: BootHookService, **kwargs: Any) -> dict[str, Any]:
...
class TestCreate:
    async def test_create_basic(self, svc):
        hook = await _make(svc, name="warm", command="echo go")
        assert hook["id"] > 0
    ...
    async def test_create_assigns_sequential_order(self, svc):
        a = await _make(svc, name="a")
        b = await _make(svc, name="b")
        c = await _make(svc, name="c")
        assert (a["order"], b["order"], c["order"]) == (0, 1, 2)

    async def test_create_independent_phases(self, svc):
        a = await _make(svc, name="pre", phase="pre_create", command="true")
        b = await _make(svc, name="post", phase="post_create", command="true")
        assert a["order"] == 0
        assert b["order"] == 0

    async def test_create_with_env(self, svc):
        hook = await _make(svc, name="e", env={"FOO": "bar"})
        assert hook["env"] == {"FOO": "bar"}

    async def test_reject_empty_name(self, svc):
        with pytest.raises(ValidationError):
            await _make(svc, name="   ")

    async def test_reject_empty_command(self, svc):
        with pytest.raises(ValidationError):
            await _make(svc, command="")

    async def test_reject_invalid_phase(self, svc):
        with pytest.raises(ValidationError):
            await _make(svc, phase="boot")

    async def test_reject_zero_timeout(self, svc):
        with pytest.raises(ValidationError):
            await _make(svc, timeout_seconds=0)

    async def test_reject_huge_timeout(self, svc):
        with pytest.raises(ValidationError):
            await _make(svc, timeout_seconds=9999)


class TestList:
    async def test_list_empty(self, svc):
        assert await svc.list("gw1", "sb1") == []

    async def test_list_returns_all(self, svc):
        await _make(svc, name="a", phase="pre_create", command="true")
        await _make(svc, name="b", phase="post_create")
        items = await svc.list("gw1", "sb1")
        assert {h["name"] for h in items} == {"a", "b"}

    async def test_list_phase_filter(self, svc):
        await _make(svc, name="a", phase="pre_create", command="true")
        await _make(svc, name="b", phase="post_create")
        items = await svc.list("gw1", "sb1", phase="post_create")
        assert [h["name"] for h in items] == ["b"]

    async def test_list_phase_validation(self, svc):
        with pytest.raises(ValidationError):
            await svc.list("gw1", "sb1", phase="bogus")

    async def test_list_scoped_by_sandbox(self, svc):
        await _make(svc, sandbox_name="sb1", name="a")
        await _make(svc, sandbox_name="sb2", name="b")
        items = await svc.list("gw1", "sb1")
...
class TestUpdate:
    async def test_update_command(self, svc):
        hook = await _make(svc)
        result = await svc.update(hook["id"], command="echo new")
        assert result["command"] == "echo new"

    async def test_update_enabled(self, svc):
        hook = await _make(svc)
        result = await svc.update(hook["id"], enabled=False)
        assert result["enabled"] is False

    async def test_update_unknown(self, svc):
        assert await svc.update(999, command="true") is None

    async def test_update_reject_empty_command(self, svc):
        hook = await _make(svc)
        with pytest.raises(ValidationError):
            await svc.update(hook["id"], command="")


class TestDelete:
    async def test_delete(self, svc):
        hook = await _make(svc)
        assert await svc.delete(hook["id"]) is True
        assert await svc.get(hook["id"]) is None

    async def test_delete_unknown(self, svc):
        assert await svc.delete(123) is False

    async def test_delete_for_sandbox(self, svc):
        await _make(svc, name="a")
        await _make(svc, name="b")
        await _make(svc, sandbox_name="sb2", name="c")
        removed = await svc.delete_for_sandbox("gw1", "sb1")
        assert removed == 2
        assert [h["name"] for h in await svc.list("gw1", "sb1")] == []
        assert [h["name"] for h in await svc.list("gw1", "sb2")] == ["c"]
...
class TestReorder:
    async def test_reorder(self, svc):
        ...
        b = await _make(svc, name="b")
        c = await _make(svc, name="c")
        result = await svc.reorder("gw1", "sb1", "post_create", [c["id"], a["id"], b["id"]])
    ...
    async def test_reorder_mismatch_raises(self, svc):
        a = await _make(svc, name="a")
        with pytest.raises(ValidationError):
            await svc.reorder("gw1", "sb1", "post_create", [a["id"], 999])

    async def test_reorder_invalid_phase(self, svc):
        with pytest.raises(ValidationError):
            await svc.reorder("gw1", "sb1", "boot", [])


class TestRunPreCreate:
    async def test_run_pre_create_success(self, svc):
        await _make(svc, name="ok", phase="pre_create", command="/bin/true")
    ...
    async def test_run_pre_create_failure_raises(self, svc):
        await _make(svc, name="bad", phase="pre_create", command="/bin/false")
    ...
    async def test_run_pre_create_invalid_command(self, svc):
        await _make(svc, name="bad", phase="pre_create", command='echo "unterminated')
        with pytest.raises(BootHookError):
            await svc.run_pre_create("gw1", "sb1", {})

    async def test_run_pre_create_skips_disabled(self, svc):
        hook = await _make(svc, name="disabled", phase="pre_create", command="/bin/false")
        await svc.update(hook["id"], enabled=False)
    ...
    async def test_run_pre_create_persists_status(self, svc):
        hook = await _make(svc, name="ok", phase="pre_create", command="/bin/true")
        await svc.run_pre_create("gw1", "sb1", {})
        refreshed = await svc.get(hook["id"])
...
class TestRunPostCreate:
    async def test_run_post_create_success(self, svc_with_provider):
        ...
        await _make(svc, name="ok", phase="post_create", command="echo done")
    ...
    async def test_run_post_create_halts_on_failure(self, svc_with_provider):
        ...
        await _make(svc, name="bad", phase="post_create", command="false")
        await _make(svc, name="next", phase="post_create", command="true")
        results = await svc.run_post_create("gw1", "sb1")
        assert len(results) == 1
    ...
    async def test_run_post_create_no_provider(self, svc_with_provider):
        ...
        await _make(svc, name="ok", phase="post_create", command="true")
        assert await svc.run_post_create("gw1", "sb1") == []

    async def test_run_post_create_skips_disabled(self, svc_with_provider):
        svc, _provider, sandbox_service = svc_with_provider
        hook = await _make(svc, name="off", phase="post_create", command="true")
        await svc.update(hook["id"], enabled=False)
        results = await svc.run_post_create("gw1", "sb1")
        assert results == []
    ...
    async def test_run_post_create_validation_error(self, svc_with_provider):
        ...
        await _make(svc, name="x", phase="post_create", command="echo hi")
        results = await svc.run_post_create("gw1", "sb1")
        assert results[0]["status"] == "failure"
...
class TestRunOne:
    ...
    async def test_run_one_post(self, svc_with_provider):
        svc, _provider, sandbox_service = svc_with_provider
        hook = await _make(svc, name="ok", phase="post_create", command="echo")
        result = await svc.run_one(hook["id"])
        assert result["status"] == "success"
        sandbox_service.exec.assert_called_once()
