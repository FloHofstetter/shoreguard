...
async def test_add_network_rule(policy_svc, mock_client):
    ...
    result = await policy_svc.add_network_rule("sb1", "new_rule", {"hosts": ["test.com"]})
...
async def test_add_network_rule_creates_section(policy_svc, mock_client):
    """Adding a rule when no network_policies section exists creates one."""
    mock_client.policies.get.return_value = _make_policy()
    mock_client.policies.update.return_value = {"revision": 2}

    await policy_svc.add_network_rule("sb1", "first_rule", {"hosts": ["test.com"]})
...
async def test_delete_network_rule(policy_svc, mock_client):
    ...
    await policy_svc.delete_network_rule("sb1", "remove")
...
async def test_delete_nonexistent_rule(policy_svc, mock_client):
    ...
    await policy_svc.delete_network_rule("sb1", "nonexistent")
...
async def test_add_rule_raises_when_no_policy(policy_svc, mock_client):
    ...
    with pytest.raises(PolicyError, match="Could not read current policy"):
        await policy_svc.add_network_rule("sb1", "rule", {"hosts": ["test.com"]})


async def test_get_policy(policy_svc, mock_client):
    """get() delegates directly to the client."""
    mock_client.policies.get.return_value = _make_policy()

    result = await policy_svc.get("sb1")

    mock_client.policies.get.assert_called_once_with("sb1")
    assert result == _make_policy()


async def test_get_effective_policy_adds_source_marker(policy_svc, mock_client):
    ...
    result = await policy_svc.get_effective("sb1")
...
async def test_get_effective_policy_non_dict_result(policy_svc, mock_client):
...
async def test_submit_analysis_forwards_to_client(policy_svc, mock_client):
    ...
    result = await policy_svc.submit_analysis(
        "sb1",
        summaries=summaries,
...
async def test_submit_analysis_default_mode(policy_svc, mock_client):
    ...
    await policy_svc.submit_analysis("sb1", summaries=[], proposed_chunks=[])

    call_kwargs = mock_client.policies.submit_analysis.call_args.kwargs
    assert call_kwargs["analysis_mode"] == ""


async def test_update_policy(policy_svc, mock_client):
    ...
    result = await policy_svc.update("sb1", {"network_policies": {}})
...
async def test_list_revisions(policy_svc, mock_client):
    """list_revisions() forwards limit/offset to the client."""
    mock_client.policies.list_revisions.return_value = [{"revision": 1}]

    result = await policy_svc.list_revisions("sb1", limit=5, offset=10)
...
async def test_apply_preset_found(policy_svc, mock_client):
    ...
    await policy_svc.apply_preset("sb1", "pypi")

    proto = mock_client.policies.update.call_args[0][1]
    assert "pypi" in proto.network_policies


async def test_apply_preset_not_found(policy_svc, mock_client):
    """apply_preset() raises NotFoundError for unknown preset name."""
    with pytest.raises(NotFoundError, match="Preset"):
        await policy_svc.apply_preset("sb1", "nonexistent-preset-xyz")
...
async def test_add_filesystem_path_rw(policy_svc, mock_client):
    """add_filesystem_path with access='rw' adds path to read_write."""
    mock_client.policies.get.return_value = _make_policy_with_fs([], [])
    mock_client.policies.update.return_value = {"revision": 2}

    await policy_svc.add_filesystem_path("sb1", "/data", "rw")
...
async def test_add_filesystem_path_moves_ro_to_rw(policy_svc, mock_client):
...
async def test_add_filesystem_path_creates_section(policy_svc, mock_client):
    """Adding path with no filesystem section creates one with include_workdir=False."""
    mock_client.policies.get.return_value = _make_policy()
    mock_client.policies.update.return_value = {"revision": 2}

    await policy_svc.add_filesystem_path("sb1", "/opt", "ro")
...
async def test_delete_filesystem_path_from_read_write(policy_svc, mock_client):
    """delete_filesystem_path removes path from read_write list."""
    mock_client.policies.get.return_value = _make_policy_with_fs([], ["/data"])
    mock_client.policies.update.return_value = {"revision": 3}

    await policy_svc.delete_filesystem_path("sb1", "/data")

    proto = mock_client.policies.update.call_args[0][1]
    assert "/data" not in list(proto.filesystem.read_write)


async def test_delete_filesystem_path_no_filesystem_section(policy_svc, mock_client):
    """delete_filesystem_path when no filesystem section is a no-op."""
    mock_client.policies.get.return_value = _make_policy()
    mock_client.policies.update.return_value = {"revision": 3}

    await policy_svc.delete_filesystem_path("sb1", "/nonexistent")

    # Should still call update (read-modify-write), just no crash
    assert mock_client.policies.update.called


async def test_update_process_policy_only_user(policy_svc, mock_client):
    """update_process_policy with only run_as_user — run_as_group not overridden."""
    mock_client.policies.get.return_value = _make_policy()
    mock_client.policies.update.return_value = {"revision": 2}

    await policy_svc.update_process_policy("sb1", run_as_user="nobody")
...
async def test_update_process_policy_only_landlock(policy_svc, mock_client):
    """update_process_policy with only landlock_compatibility."""
    mock_client.policies.get.return_value = _make_policy()
    mock_client.policies.update.return_value = {"revision": 2}

    await policy_svc.update_process_policy("sb1", landlock_compatibility="best_effort")
...
class TestAddFilesystemPathMutationKill:
    ...
    async def test_ro_path_not_in_rw(self, policy_svc, mock_client):
        """A read_only path must NOT appear in read_write."""
        mock_client.policies.get.return_value = _make_policy_with_fs([], [])
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("sb1", "/etc", "ro")

        proto = mock_client.policies.update.call_args[0][1]
        assert list(proto.filesystem.read_only) == ["/etc"]
        assert list(proto.filesystem.read_write) == []

    async def test_rw_path_not_in_ro(self, policy_svc, mock_client):
        """A read_write path must NOT appear in read_only."""
        mock_client.policies.get.return_value = _make_policy_with_fs([], [])
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("sb1", "/var", "rw")

        proto = mock_client.policies.update.call_args[0][1]
        assert list(proto.filesystem.read_write) == ["/var"]
        assert list(proto.filesystem.read_only) == []

    async def test_move_rw_to_ro(self, policy_svc, mock_client):
        """Adding a path as ro that exists in rw must remove it from rw."""
        mock_client.policies.get.return_value = _make_policy_with_fs([], ["/data"])
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("sb1", "/data", "ro")

        proto = mock_client.policies.update.call_args[0][1]
        assert "/data" in list(proto.filesystem.read_only)
        assert "/data" not in list(proto.filesystem.read_write)

    async def test_dedup_ro_path(self, policy_svc, mock_client):
    ...
    async def test_dedup_rw_path(self, policy_svc, mock_client):
    ...
    async def test_creates_filesystem_section_with_defaults(self, policy_svc, mock_client):
        """When no filesystem section, creates one with correct defaults."""
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("sb1", "/opt", "rw")
    ...
    async def test_preserves_existing_paths(self, policy_svc, mock_client):
    ...
    async def test_preserves_include_workdir(self, policy_svc, mock_client):
    ...
    async def test_calls_update_with_sandbox_name(self, policy_svc, mock_client):
        """update must be called with the correct sandbox name."""
        mock_client.policies.get.return_value = _make_policy_with_fs([], [])
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("my-sandbox", "/etc", "ro")

        call_args = mock_client.policies.update.call_args
        assert call_args[0][0] == "my-sandbox"

    async def test_calls_get_with_sandbox_name(self, policy_svc, mock_client):
    ...
    async def test_no_policy_raises_error(self, policy_svc, mock_client):
    ...
    async def test_access_ro_default_behavior(self, policy_svc, mock_client):
        """Non-'rw' access value should add to read_only (else branch)."""
        mock_client.policies.get.return_value = _make_policy_with_fs([], [])
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_filesystem_path("sb1", "/etc", "readonly")
...
class TestDeleteFilesystemPathMutationKill:
    ...
    async def test_deletes_from_ro_only(self, policy_svc, mock_client):
    ...
    async def test_deletes_from_both_lists(self, policy_svc, mock_client):
    ...
    async def test_nonexistent_path_is_noop(self, policy_svc, mock_client):
    ...
    async def test_no_filesystem_section(self, policy_svc, mock_client):
        """When no filesystem section exists, should still call update."""
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 3}

        await policy_svc.delete_filesystem_path("sb1", "/etc")

        mock_client.policies.update.assert_called_once()

    async def test_calls_update_with_correct_sandbox(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy_with_fs(["/etc"], [])
        mock_client.policies.update.return_value = {"revision": 3}

        await policy_svc.delete_filesystem_path("my-sb", "/etc")

        assert mock_client.policies.update.call_args[0][0] == "my-sb"

    async def test_empty_policy_raises(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = {"policy": None}
...
class TestUpdateProcessPolicyMutationKill:
    ...
    async def test_only_group(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.update_process_policy("sb1", run_as_group="wheel")

        proto = mock_client.policies.update.call_args[0][1]
        assert proto.process.run_as_group == "wheel"
        assert proto.process.run_as_user == ""

    async def test_no_args_leaves_process_empty(self, policy_svc, mock_client):
        """Calling with no args should create empty process dict but not set values."""
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.update_process_policy("sb1")

        proto = mock_client.policies.update.call_args[0][1]
        assert proto.process.run_as_user == ""
        assert proto.process.run_as_group == ""

    async def test_user_and_group_together(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.update_process_policy("sb1", run_as_user="app", run_as_group="app")

        proto = mock_client.policies.update.call_args[0][1]
        assert proto.process.run_as_user == "app"
        assert proto.process.run_as_group == "app"

    async def test_landlock_and_user(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.update_process_policy(
            "sb1", run_as_user="root", landlock_compatibility="strict"
        )
    ...
    async def test_existing_process_section_preserved(self, policy_svc, mock_client):
        ...
        await policy_svc.update_process_policy("sb1", run_as_group="newgroup")

        proto = mock_client.policies.update.call_args[0][1]
        assert proto.process.run_as_user == "existing"
        assert proto.process.run_as_group == "newgroup"

    async def test_calls_correct_sandbox(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.update_process_policy("prod-sandbox", run_as_user="nobody")
...
class TestApplyPresetMutationKill:
    ...
    async def test_preset_merges_into_existing_rules(self, policy_svc, mock_client):
    ...
    async def test_preset_creates_network_section(self, policy_svc, mock_client):
    ...
    async def test_not_found_preset_raises(self, policy_svc, mock_client):
        with pytest.raises(NotFoundError, match="not found"):
            await policy_svc.apply_preset("sb1", "totally-nonexistent-preset-abc123")

    async def test_not_found_preset_does_not_call_get(self, policy_svc, mock_client):
    ...
    async def test_calls_correct_sandbox(self, policy_svc, mock_client):
        ...
        await policy_svc.apply_preset("my-sb", "pypi")
...
class TestDeleteNetworkRuleMutationKill:
    ...
    async def test_removes_correct_key(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy(
            {"rule_a": {"hosts": ["a.com"]}, "rule_b": {"hosts": ["b.com"]}}
        )
        mock_client.policies.update.return_value = {"revision": 3}

        await policy_svc.delete_network_rule("sb1", "rule_a")

        proto = mock_client.policies.update.call_args[0][1]
        assert "rule_a" not in proto.network_policies
        assert "rule_b" in proto.network_policies

    async def test_nonexistent_key_is_noop(self, policy_svc, mock_client):
    ...
    async def test_no_network_section(self, policy_svc, mock_client):
        """Deleting from empty network_policies must not raise."""
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 3}

        await policy_svc.delete_network_rule("sb1", "anything")
    ...
    async def test_calls_correct_sandbox(self, policy_svc, mock_client):
        ...
        await policy_svc.delete_network_rule("prod", "r")
...
class TestUpdateMutationKill:
    ...
    async def test_returns_refetched_policy(self, policy_svc, mock_client):
    ...
    async def test_calls_update_then_get(self, policy_svc, mock_client):
        """update must call client.policies.update THEN client.policies.get."""
        mock_client.policies.update.return_value = {"revision": 5}
        mock_client.policies.get.return_value = _make_policy()

        await policy_svc.update("sb1", {"network_policies": {}})

        mock_client.policies.update.assert_called_once()
        # get is called with sandbox name
        mock_client.policies.get.assert_called_once_with("sb1")

    async def test_passes_correct_sandbox_to_update(self, policy_svc, mock_client):
        mock_client.policies.update.return_value = {"revision": 5}
        mock_client.policies.get.return_value = _make_policy()

        await policy_svc.update("my-sandbox", {"status": "loaded"})

        assert mock_client.policies.update.call_args[0][0] == "my-sandbox"

    async def test_converts_dict_to_proto(self, policy_svc, mock_client):
        """The dict must be converted to protobuf before calling client.update."""
        mock_client.policies.update.return_value = {"revision": 5}
        mock_client.policies.get.return_value = _make_policy()

        await policy_svc.update("sb1", {"network_policies": {"r1": {"hosts": ["a.com"]}}})
...
class TestDiffRevisionsMutationKill:
    ...
    async def test_returns_correct_structure(self, policy_svc, mock_client):
    ...
    async def test_versions_not_swapped(self, policy_svc, mock_client):
        ...
        result = await policy_svc.diff_revisions("sb1", 10, 20)
    ...
    async def test_calls_get_version_with_correct_args(self, policy_svc, mock_client):
        mock_client.policies.get_version.return_value = {"policy": {}, "revision": {}}

        await policy_svc.diff_revisions("sb1", 3, 7)
    ...
    async def test_missing_keys_return_none(self, policy_svc, mock_client):
...
class TestListRevisionsMutationKill:
    ...
    async def test_returns_client_result_directly(self, policy_svc, mock_client):
        ...
        result = await policy_svc.list_revisions("sb1")
...
class TestReadModifyWriteMutationKill:
    ...
    async def test_empty_policy_dict_raises(self, policy_svc, mock_client):
        """An empty dict (falsy) should raise PolicyError."""
        mock_client.policies.get.return_value = {"policy": {}}

        with pytest.raises(PolicyError, match="Could not read current policy"):
            await policy_svc._read_modify_write("sb1", lambda p: None)

    async def test_none_policy_raises(self, policy_svc, mock_client):
    ...
    async def test_mutation_fn_receives_policy_dict(self, policy_svc, mock_client):
        ...
        await policy_svc._read_modify_write("sb1", capture)

        assert len(received) == 1
        assert "status" in received[0]
        assert "network_policies" in received[0]

    async def test_returns_update_result(self, policy_svc, mock_client):
        ...
        result = await policy_svc._read_modify_write("sb1", lambda p: None)
...
class TestAddNetworkRuleMutationKill:
    ...
    async def test_overwrites_existing_key(self, policy_svc, mock_client):
        ...
        await policy_svc.add_network_rule("sb1", "my_rule", {"name": "new rule"})

        proto = mock_client.policies.update.call_args[0][1]
        assert proto.network_policies["my_rule"].name == "new rule"

    async def test_correct_sandbox_name(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_network_rule("test-sb", "r", {"hosts": []})

        assert mock_client.policies.update.call_args[0][0] == "test-sb"

    async def test_creates_network_section_if_missing(self, policy_svc, mock_client):
        mock_client.policies.get.return_value = _make_policy()
        mock_client.policies.update.return_value = {"revision": 2}

        await policy_svc.add_network_rule("sb1", "new", {"hosts": ["x.com"]})
